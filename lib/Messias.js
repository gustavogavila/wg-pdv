...
class Messias {
  ...
  handleRota() {
    ...
    const route = this.rotas.filter(route =>
  ...
  verificarSession(componente = null) {
    ...
    if (componente) this.irPara(componente);
    else return false;
  }
  limparSession() {
...
getInstancia = (str, nome) => {
  ...
  match.map(reg => {
    ...
    str = str.replace(
      partes[0],
      `on${evento}="Messias.get('${nome}').${metodo}(${parametros})"`
    );
  });
  return str;
};
...
gerarArray = (str, objeto) => {
  ...
  let corpo = partes[3];
  let newCorpo = "";
  let arrays = getKey(argumentos[1], objeto)
